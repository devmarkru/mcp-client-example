...
Пример MCP (**model context protocol**) клиента на **Spring AI**. Требует
наличия [MCP-сервера](https://github.com/devmarkru/mcp-server-example), запущенного на
`localhost:8081`, но это можно переопределить в `application.yml`.
...
- `OPEN_AI_BASE_URL` (опционально) - позволяет переопределить эндпоинт LLM, к которому происходит подключение (при
  использовании другой LLM с совместимым протоколом)
